...
void OverlayPainter::draw(ID2D1RenderTarget* rt, const AnalysisResult& result)
{
...

    {
        ...
        if (expire > 0 && now_ms < expire) {
            ...
            break_alert_expire_ms_.store(0);
        }
    }
    ...
    const float py = kPanelMargin + 48.0f;
...
{
    rt->DrawText(text.c_str(), static_cast<UINT32>(text.size()), fmt,
...
void OverlayPainter::draw_stats_panel(ID2D1RenderTarget* rt)
{
    ...
    draw_text(rt, L"Session Stats",
              px + 10.0f, py + 6.0f, 120.0f, 16.0f, fmt_label_.Get(), brush_gray_.Get());
    ...
    draw_text(rt, buf, sx, sy, 140.0f, 16.0f, fmt_label_.Get(), brush_white_.Get()); sy += 20.0f;
    ...
    draw_text(rt, buf, sx, sy, 140.0f, 16.0f, fmt_label_.Get(), posture_brush); sy += 20.0f;

    swprintf_s(buf, L"Events    : %d", snap.event_count);
    draw_text(rt, buf, sx, sy, 140.0f, 16.0f, fmt_label_.Get(),
              snap.event_count > 0 ? brush_orange_.Get() : brush_white_.Get()); sy += 20.0f;
    ...
    rt->DrawLine(D2D1::Point2F(chart_x, mid_y), D2D1::Point2F(chart_r, mid_y),
    ...
    for (int i = 1; i < n; ++i) {
        ...
        rt->DrawLine(D2D1::Point2F(x0, y0), D2D1::Point2F(x1, y1), line_brush, 1.5f);
    }

    const float lx = chart_x + step * (n - 1);
    const float ly = chart_b - (ch.back() / 100.0f) * kChartH;
    rt->FillEllipse(D2D1::Ellipse(D2D1::Point2F(lx, ly), 3.5f, 3.5f), brush_chart_line_.Get());
}
...
void OverlayPainter::draw_break_alert(ID2D1RenderTarget* rt)
{
    ...
    rt->FillRectangle(D2D1::RectF(0, 0, size.width, size.height), brush_break_bg_.Get());

    constexpr float kPW = 400.0f;
    constexpr float kPH = 200.0f;
    const D2D1_ROUNDED_RECT panel = D2D1::RoundedRect(
        D2D1::RectF(cx - kPW * 0.5f, cy - kPH * 0.5f,
                    cx + kPW * 0.5f, cy + kPH * 0.5f), 14.0f, 14.0f);
    rt->FillRoundedRectangle(panel, brush_break_panel_.Get());
    rt->DrawRoundedRectangle(panel, brush_orange_.Get(), 1.5f);

    rt->FillEllipse(D2D1::Ellipse(D2D1::Point2F(cx, cy - 60.0f), 20.0f, 20.0f), brush_orange_.Get());
    draw_text(rt, L"!", cx - 20.0f, cy - 74.0f, 40.0f, 30.0f, fmt_calib_count_.Get(), brush_white_.Get());

    draw_text(rt, L"Time for a Break!",
              cx - kPW * 0.5f, cy - 26.0f, kPW, 28.0f, fmt_break_title_.Get(), brush_orange_.Get());
    draw_text(rt, L"You've been studying hard. Please rest your eyes",
              cx - kPW * 0.5f, cy + 12.0f, kPW, 20.0f, fmt_break_msg_.Get(), brush_gray_.Get());
    draw_text(rt, L"and take a short stretch break.",
              cx - kPW * 0.5f, cy + 34.0f, kPW, 20.0f, fmt_break_msg_.Get(), brush_gray_.Get());
    draw_text(rt, L"(This message will disappear automatically)",
              cx - kPW * 0.5f, cy + 72.0f, kPW, 16.0f, fmt_label_.Get(), brush_gray_.Get());
}
...
void OverlayPainter::draw_calibration(ID2D1RenderTarget* rt, int countdown)
{
    ...
    if (countdown > 0) {
        // 캘리브레이션 진행 중 — 한국어 안내 메시지
        draw_text(rt, L"공부 자세를 잊아주세요.",
                  cx - kPW * 0.5f, cy - 36.0f, kPW, 30.0f,
                  fmt_calib_count_.Get(), brush_white_.Get());
        draw_text(rt, L"판단의 기준점이 됩니다.",
                  cx - kPW * 0.5f, cy + 4.0f, kPW, 26.0f,
                  fmt_calib_msg_.Get(), brush_gray_.Get());
    } else {
        // countdown == 0 — 완료 메시지
        draw_text(rt, L"기준 자세가 설정되었습니다.",
                  cx - kPW * 0.5f, cy - 28.0f, kPW, 30.0f,
                  fmt_calib_count_.Get(), brush_green_.Get());
        draw_text(rt, L"공부를 시작합니다.",
                  cx - kPW * 0.5f, cy + 10.0f, kPW, 26.0f,
                  fmt_calib_msg_.Get(), brush_gray_.Get());
    }
}
...
void OverlayPainter::draw_status_badge(ID2D1RenderTarget* rt, const AnalysisResult& result)
{
    ...
    const float dot_cx = bx + 20.0f;
    const float dot_cy = by + kBH / 2.0f;
    draw_dot(rt, dot_cx, dot_cy, 8.0f, circle_brush);
    draw_text(rt, label, dot_cx + 16.0f, by + 2.0f, kBW - 44.0f, kBH - 4.0f,
